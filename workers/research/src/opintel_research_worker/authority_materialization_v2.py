"""Deterministic materialisation of one v2 (@4) sampled-slot execution approval.

AUTHORIZE_SLOTS02_06_PHASE1_M1_V2_EXECUTION differs from the Window 2 Slot 01
path in three ways and only three:

* the owner approved BY REFERENCE to an immutable package (sha256
  d28e42594c943debf0144b3f5ecce57589760881e579ebbf52f6984d8150677a), not a
  regex-parseable statement -- so the bound identifiers, window and slot come
  from the sealed artefacts, not from statement text;
* the M1 ceilings come from the sealed execution-ceiling envelope
  (execution_ceilings_sha256 083b70520f161d4c21aa72cc221cb063dc2b99cf19ab15caa9c0cd7011b0506b),
  additionally clamped by the owner's batch-wide hard bounds with no cross-slot
  borrowing;
* the deployed runtime revision is the v2 protocol-upgrade image, accepted only
  because the package's sealed deployment-evidence binds that exact digest.

Everything else -- predecessor binding from SSM, dormant preconditions, the
final dry-run through the exact activator-side applicator -- is identical to the
v1 materialiser and is reused from :mod:`authority_materialization`.
"""
...
PACKAGE_SHA256 = "d28e42594c943debf0144b3f5ecce57589760881e579ebbf52f6984d8150677a"
...
def _verify_sealed_artifacts(inputs: MaterializationInputsV2) -> dict[str, Any]:
    _require(
    ...
    _require(
        package.get("proposed_decision_event") == "AUTHORIZE_SLOTS02_06_PHASE1_M1_V2_EXECUTION",
        "authorization package is not the Slots 02-06 batch gate",
    )
    ...
    _require(
        decision.get("decision_event") == "AUTHORIZE_SLOTS02_06_PHASE1_M1_V2_EXECUTION"
        and decision.get("authorization_package", {}).get("sha256") == PACKAGE_SHA256,
        "owner-decision record does not bind this exact authorization package",
    )
...
def _predecessor_v2(current_raw: str, authorized_release_id: UUID) -> dict[str, object]:
...
def _runtime_v2(env: dict[str, str], deployed_digest: str) -> dict[str, str]:
    values: dict[str, str] = {}
    ...
    _require(
        ...
        "deployed activator runtime revision is not the package-bound v2 image digest",
    )
    return values


def materialize_v2(inputs: MaterializationInputsV2) -> MaterializationResult:
    _require(inputs.slot_number in _BATCH_SLOTS, "slot number is not in the authorised 02-06 batch")
    ...
    approval_id = uuid5(
        NAMESPACE_URL, f"m67.phase1.v2-approval:{PACKAGE_SHA256}:{inputs.slot_number}"
    )
    authorized_release_id = uuid5(
        NAMESPACE_URL, f"m67.phase1.v2-release:{PACKAGE_SHA256}:{inputs.slot_number}"
    )
    predecessor = _predecessor_v2(inputs.current_research_release_raw, authorized_release_id)
    runtime = _runtime_v2(inputs.deployed_activator_environment, deployed_digest)
    ...
    if not isinstance(pred_obj, LiveResearchPermissionRelease):
        raise AuthorityMaterializationError(
            "v2 materialisation requires a consumed predecessor research-release, "
    ...
    try:
        ...
        raise AuthorityMaterializationError(
            f"materialised v2 approval fails schema {APPROVAL_SCHEMA_V4}: {error}"
        ) from error
    ...
    # activator-side applicator, exactly as the v1 materialiser does.
    ...
    try:
        ...
        raise AuthorityMaterializationError(
            f"production applicator rejects the materialised v2 envelope: {error}"
        ) from error
    _require(created is True, "production applicator did not treat the v2 envelope as fresh")
    _require(
        ...
        "production applicator built a v2 release outside the exact authority",
    )
    _require(
        release_execution_ceilings_sha256(release)
        == release_execution_ceilings_sha256(release),
        "v2 execution-ceilings sha256 is not stable",
    )
