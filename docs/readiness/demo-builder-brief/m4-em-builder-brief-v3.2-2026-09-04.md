# E+M Emergency Air Conditioning — an intake opportunity

_Show on every screen:_ **Simulation · not connected to E+M**
...
## Screen 1 — One way a generic structured intake flow could organize incoming commercial HVAC requests

> Your public pages show broad commercial-HVAC service labels only. We built a short simulation of a generic structured intake flow that prepares a commercial request for human follow-up - not built around your business. We don't know your current internal process, systems, staffing, or response performance.
...
One calm page - no wizard. Every value below is an **illustrative simulation choice**, not a fact about E+M and not something they told us.
...
2. **Structured context** — The simulation turns the form answers into a commercial request, organized so a person can pick it up.
...
> Intake coordinator\*
>
> \* Morgan is a fictional demo placeholder, not a real person. We don't know how E+M assigns or handles requests internally - the role is illustrative only.
...
| key | value |
|---|---|
| envelope_sha256 | `2c35afc05382f3f26e3cf279e0736fdb09caa9145656420f636510ea218eaf98` |
...
| demo_mode | `GENERIC_CAPABILITY_DEMO` |
| underlying_experience_sha256 | `59457dcd3a604c32baaecb6d9e1179f01b9488ebbc928afff51107e5ffd62d52` |
| underlying_brief_sha256 | `1afe44ad49be2c3e240e3d69571242fd36dd93ef30f0745eeb2cb188dfbdd05d` |
...
- `service_need` — hint "Which need applies" — Your public pages describe commercial HVAC work, so the simulation asks which need applies.
  - M4 evidence-derived option labels: ['Request Service', 'Commercial AC', 'other', 'unknown']
...
- `service_location` — hint "Example simulation location" — Your public pages reference a service area, so the simulation asks about location - without making any coverage promise.
  - M4 evidence-derived option labels: ['Service Areas', 'other', 'unknown']
...
**Scenario framing (per-anchor story):** a commercial request, organized so a person can pick it up
