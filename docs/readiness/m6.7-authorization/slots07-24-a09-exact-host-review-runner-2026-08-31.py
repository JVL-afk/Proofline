"""Bounded A-09 exact-host access review for Slots 07-24.

OWNER DIRECTIVE - AUTHORIZE BOUNDED SLOTS07_24 MECHANICAL ENABLEMENT / LINK 9
(reviewed batch package sha256
41cf105feb2ab1b21038803583867d56ea28e0a3dc99f46e5df91f7f66998251):
perform the existing unchanged bounded A-09 exact-host process for the eighteen
named frozen mappings only. Read-only, no redirects, identity user agent, small
byte cap, content-minimised evidence, one same-host legal page. Never crawls.
Applies the existing acceptance criteria and, only when satisfied, emits an
APPROVED_FOR_BOUNDED_FIRST_PARTY_PUBLIC_RESEARCH decision row.

This runner is byte-identical to
docs/readiness/m6.7-deployment/slots02-06-a09-exact-host-review-runner-2026-08-30.py
(sha256 e4ef8e510d3b369ca19a7b447459ddb60d68f915ea7c2160947f8932a96e5db8) except
for the SLOTS list (07-24) and the bound authorization_package_sha256.
"""
...
def review_host(slot: int, business: str, host: str) -> dict[str, object]:
    ...
    # The prior 2026-08-25 / 2026-08-30 processes recommended APPROVE with NO
    # same-host legal path in several cases -- a legal page is collected as
...
def main() -> None:
    out = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("a09_slots07_24_result.json")
