"""M6.8-3 **Attempt 7** synthetic live-provider certification - local harness.

Owner authorization 2026-09-02 ("COMPLETE M6.8-3 REMEDIATION + NEXT CERTIFICATION
ATTEMPT"): diagnose the Attempt-6 residuals, make the minimum deterministic /
prompt / contract fixes, validate offline, freeze a new configuration/key,
execute the next bounded live claude-sonnet-5 certification attempt, return the
report and stop. Do not begin M6.8-4 even on CERTIFIED_SAFE.

Attempt 7 keeps the exact provider configuration of Attempts 5/6 (``claude-sonnet-5``,
thinking disabled, N=1, ``max_output_tokens=8000``, timeout 30 s, 0 retries,
81-call ceiling, 648000 aggregate token ceilings, USD 10.00 hard ceiling, frozen
corpus ``comm.m6_8_3_synthetic_corpus@1`` manifest 282cdd5a...). The deterministic
side is the post-remediation ``contract="v2"`` contract:

  * prompt template ``comm.prompt_template.first_contact@7``  (sha 149be100...)
  * output validator ``comm.output_validator@3``
  * CTA parser        ``comm.cta_parser@2``  (unchanged this round)
  * certification / drift policy ``comm.provider_certification@2``

The semantic-envelope truth content, M1-M5, UNKNOWN semantics, prohibited-claim
policy, the frozen synthetic corpus, the model, the 8000 ceiling, N=1,
``minimum_candidate_pass_rate=0.80``, ``maximum_manifest_mismatch_rate=0.00``,
the zero-tolerance safety set, and the human-review / contact / send gates are
all UNCHANGED. ``@3`` ADVISORY findings (``no_company_specific_evidence``,
``provider_manifest_type_mismatch``) are reported but never fail a candidate or
the run.

Certification identity (section D) is two-stage and the two are equal when there
is no served-model identity drift:
  * PRE_RUN_CONFIG_ID   - the key sha computed from the frozen tuple before any
    call, binding the INTENDED served-model identity (claude-sonnet-5), NOT an
    "unobserved" placeholder.
  * FINAL_CERTIFICATION_KEY - the key sha in the report after the run, binding
    the model the adapter actually observed.
...
delivery, no send. Does NOT begin M6.8-4.
    ...
    python scripts/run_m68_3_attempt7_certification.py            # dry run
    python scripts/run_m68_3_attempt7_certification.py --execute   # 81 real calls (+1 probe)
"""
...
_ATTEMPT7_CONFIG = GenerationConfig(thinking="disabled", max_output_tokens=8000)
_ATTEMPT7_BOUNDS = ATTEMPT7_CALL_BOUNDS
_PROMPT_BUILDER = build_certification_prompt_bundle_v7
_VALIDATOR_CONTRACT = "v2"
_VALID_CLAIM_TYPES = {c.value for c in ClaimType}
_VALID_STRENGTHS = {s.value for s in FactStrength}
_REPEATS = 9
_ND_REPEATS = 3
_ATTEMPT_LABEL = "attempt7-remediated-v3-n1-8k"

# The frozen Attempt-7 certification identity (section D). PRE_RUN_CONFIG_ID and
# FINAL_CERTIFICATION_KEY are the same sha because the key binds the intended
# served-model identity (claude-sonnet-5) consistently. Preflight fails closed
# unless the runner computes exactly this.
_EXPECTED_KEY_SHA = "52114bcf9acb8fda9ef8cfab7e2e7313090f0e51e1701e64170ed5d66fcc39a2"
_EXPECTED_PROMPT_SHA = "149be1004645d27e2ec59d03d619d629bb73122424fcb62b5819aecfc66738f4"
...
def _plan_summary() -> str:
    ...
    lines = [
        ...
        f"  config            : thinking={_ATTEMPT7_CONFIG.thinking or 'provider-default'} "
        f"N={b.candidates_per_call} max_output_tokens={_ATTEMPT7_CONFIG.max_output_tokens} "
        f"config_hash={_ATTEMPT7_CONFIG.config_hash()[:16]}...",
        f"  prompt template   : {_PROMPT_BUILDER(distinctive[0].envelope).template_id}",
        f"  validator contract: {_VALIDATOR_CONTRACT} "
        "(comm.output_validator@3 / comm.cta_parser@2 / comm.provider_certification@2)",
...
def _preflight(*, require_key: bool) -> tuple[list[str], dict[str, object]]:
    ...
    # adapter / model / config / key binds the frozen Attempt-7 tuple
    adapter = AnthropicProviderAdapter("sk-ant-preflight-not-a-real-key", config=_ATTEMPT7_CONFIG)
    runner = CertificationRunner(
        adapter,
        bounds=_ATTEMPT7_BOUNDS,
    ...
    if not (
        ...
        and key_obj.prompt_template_id == PROMPT_TEMPLATE_ID_CERT_V7
        and key_obj.prompt_template_sha256 == _EXPECTED_PROMPT_SHA
        and key_obj.output_validator_version == "comm.output_validator@3"
        and key_obj.cta_parser_version == "comm.cta_parser@2"
        and key_obj.observed_model_identity == PINNED_MODEL
        and key_obj.generation_config_hash == _EXPECTED_CONFIG_HASH
    ):
        raise SystemExit(
            "PREFLIGHT FAIL: certification key members do not match the frozen Attempt-7 tuple"
        )
    if key_sha != _EXPECTED_KEY_SHA:
        raise SystemExit(
            f"PREFLIGHT FAIL: certification key sha {key_sha} != frozen {_EXPECTED_KEY_SHA}. "
            "The code/prompt/config is not the frozen Attempt-7 state. STOP."
        )
    checks.append(
    ...
    if _ATTEMPT7_BOUNDS.candidates_per_call != 1:
        raise SystemExit("PREFLIGHT FAIL: Attempt 7 bounds do not request exactly 1 candidate")
    if (
        _ATTEMPT7_BOUNDS.max_output_tokens_per_call != 8000
        or _ATTEMPT7_CONFIG.max_output_tokens != 8000
    ):
        raise SystemExit("PREFLIGHT FAIL: Attempt 7 output ceiling is not 8000")
    if _ATTEMPT7_BOUNDS.hard_usd_ceiling != "10.00" or _ATTEMPT7_BOUNDS.automatic_retries != 0:
        raise SystemExit("PREFLIGHT FAIL: Attempt 7 USD ceiling / retry policy changed")
    ...
    _need = (
        ...
        # conservative generation targets + self-check (section B.1)
        "50 characters or fewer",
        "90 to 115 words",
        "Count the words in your body yourself",
        "Prefer omitting a licensed detail over exceeding the bound",
        # unchanged safety guidance
        "RESPONSE_COMMITMENT",
        "DISCLOSURE, not a FACT",
        "the framing verb licenses nothing",
    )
    missing = [s for s in _need if s not in tmpl]
    if missing:
        raise SystemExit(f"PREFLIGHT FAIL: @7 template missing required text: {missing}")
    if "at most 130 words" not in tmpl or "at most 60 characters" not in tmpl:
        raise SystemExit("PREFLIGHT FAIL: @7 must NOT raise the hard validator caps")
    for enum_val in (*_VALID_CLAIM_TYPES, *_VALID_STRENGTHS):
        if enum_val not in tmpl:
            raise SystemExit(f"PREFLIGHT FAIL: @7 template omits the enum value {enum_val!r}")
    checks.append(
        f"@7 template: N=1, hard caps unchanged (<=60-char subject / <=130-word body / one "
        f"CTA), conservative targets (<=50 / 90-115) + self-count-and-trim + prefer-omission, "
        f"do-not-restate-RESPONSE_COMMITMENT, sim-mechanics=DISCLOSURE, framing-verb-licenses-"
        f"nothing, all {len(_VALID_CLAIM_TYPES)} claim_type + {len(_VALID_STRENGTHS)} "
        f"asserted_strength enums + the entry field list; output ceiling 8000/call; bounds "
        f"{_ATTEMPT7_BOUNDS.max_provider_calls} calls / "
        f"{_ATTEMPT7_BOUNDS.aggregate_input_token_ceiling} agg-in / "
        f"{_ATTEMPT7_BOUNDS.aggregate_output_token_ceiling} agg-out; USD "
        f"{_ATTEMPT7_BOUNDS.hard_usd_ceiling} hard; floor 0.80; manifest ceiling 0.00; "
        f"counters start 0/0/0"
    )

    # Attempt-7 compatibility probe: exactly one, counted + disclosed separately.
    if require_key:
        probe = AnthropicProviderAdapter(_read_anthropic_key(), config=_ATTEMPT7_CONFIG)
        s01 = corpus.specs[0]
        ...
        try:
            ...
            raise SystemExit(
                f"PREFLIGHT FAIL: Attempt 7 compatibility probe failed "
        ...
        checks.append(
            ...
            f"reached comm.output_validator@3 (passed={vr.passed}, findings="
...
def _markdown_report(
    ...
    a("")
    a(
        "Immutable. Owner-authorized 2026-09-02 ('COMPLETE M6.8-3 REMEDIATION + NEXT "
        "CERTIFICATION ATTEMPT'). Option A local execution. No AWS surface. No contact "
        "resolution, no delivery, no send. M6.8-4 NOT started even on CERTIFIED_SAFE."
    )
    a("")
    a(
        "**Attempt 7** = same provider configuration as Attempts 5/6 (`claude-sonnet-5`, "
        "thinking **disabled**, **N=1**, `max_output_tokens=8000`, timeout 30 s, 81 calls / "
        "USD 10.00 / 0 retries). The deterministic side is the post-remediation "
        '`contract="v2"` contract: prompt `comm.prompt_template.first_contact@7`, '
        "`comm.output_validator@3`, `comm.cta_parser@2`, `comm.provider_certification@2`. "
        ...
        "UNCHANGED. `@3` ADVISORY findings (`no_company_specific_evidence`, "
        "`provider_manifest_type_mismatch`) are reported but never fail a candidate or the run. "
        "Drift policy `@2`: run-to-run prose / advisory variation is "
        "STOCHASTIC_CANDIDATE_VARIATION (not invalidating); only served-model / config / corpus "
        "identity drift invalidates; the run-wide zero-tolerance check is unchanged. "
        "Independent configuration; do NOT aggregate metrics with Attempts 1-6."
    )
    a("")
    a("## Preflight")
    for c in checks:
    ...
    a(f"- certification policy: `{r.certification_version}`")  # type: ignore[attr-defined]
    a(f"- **PRE_RUN_CONFIG_ID: `{_EXPECTED_KEY_SHA}`** (frozen before any call)")
    a(f"- **FINAL_CERTIFICATION_KEY: `{key_sha}`** (bound after the run)")
    a(f"- identities equal (no served-model identity drift): {key_sha == _EXPECTED_KEY_SHA}")
    a("")
    a("## Corpus")
...
def main() -> int:
    ...
    key = _read_anthropic_key()
    corpus = build_corpus()
    adapter = AnthropicProviderAdapter(key, config=_ATTEMPT7_CONFIG)
    runner = CertificationRunner(
        adapter,
        bounds=_ATTEMPT7_BOUNDS,
        price_table=CONFIRMED_SONNET5_PRICE_TABLE,
    ...
    print(
        f"\nexecuting up to {_ATTEMPT7_BOUNDS.max_provider_calls} real claude-sonnet-5 calls "
    ...
    print(f"PRE_RUN_CONFIG_ID     : {_EXPECTED_KEY_SHA}")
    print(f"FINAL_CERTIFICATION_KEY: {report.certification_key_sha256}")
    print(f"identities equal      : {report.certification_key_sha256 == _EXPECTED_KEY_SHA}")
